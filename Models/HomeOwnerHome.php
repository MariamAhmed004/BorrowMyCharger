...
class HomeOwnerHome {
    ...
    /**
     * Get rejected booking requests count
     */
    public function getRejectedBookingRequestsCount() {
        $sql = "SELECT COUNT(*) FROM Pro_Booking b
                JOIN Pro_ChargePoint cp ON b.charge_point_id = cp.charge_point_id
                WHERE cp.user_id = :user_id AND b.booking_status_id = 3";
        $stmt = $this->dbHandle->prepare($sql);
        $stmt->bindParam(':user_id', $this->userId, PDO::PARAM_INT);
        $stmt->execute();
        $count = $stmt->fetchColumn();
     ...
     */
   public function getUpcomingBookings($page = 1, $limit = 5) {
    $today = date('Y-m-d');
    $offset = ($page - 1) * $limit;
